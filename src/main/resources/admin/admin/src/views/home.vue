<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('ceshifenxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{ceshifenxiCount}}</div>
					<div class="name">测试分析总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 1 -->
		<div class="type1">
			<div id="ceshifenxiChart1" class="echarts1 animate__animated" v-if="isAuth('ceshifenxi','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getceshifenxiCount();
		if(this.isAuth('ceshifenxi','首页统计')){
			this.ceshifenxiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'ceshifenxiChart1',css:'animate__fadeInUp'},
			]
		...
		getceshifenxiCount() {
			this.$http({
				url: `ceshifenxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.ceshifenxiCount = data.data
				}
			})
		},
// 1234 竖
		ceshifenxiChat1() {
			this.$nextTick(()=>{
				...
				var ceshifenxiChart1 = echarts.init(document.getElementById("ceshifenxiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						ceshifenxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							ceshifenxiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
