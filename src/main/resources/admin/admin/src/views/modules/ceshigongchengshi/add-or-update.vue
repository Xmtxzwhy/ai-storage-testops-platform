		...
		data() {
			...
			return {
				...
				rules: {
					...
					gongchengshiming: [
						{ required: true, message: '工程师名不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `ceshigongchengshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ceshigongchengshiCrossAddOrUpdateFlag = false;
											this.parent.search();
