			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","textAlign":"right","display":"block","width":"auto","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">用例名称</label>
						<el-input v-model="searchForm.yonglimingcheng" placeholder="用例名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('ceshiyongli','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#FFFFFF","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('ceshiyongli','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="yonglimingcheng"
						label="用例名称">
						<template slot-scope="scope">
							{{scope.row.yonglimingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' prop="yongliwenjian" label="用例文件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.yongliwenjian" type="text" size="small" @click="download(scope.row.yongliwenjian)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('ceshiyongli','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('ceshiyongli','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('ceshiyongli','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.yonglimingcheng!='' && this.searchForm.yonglimingcheng!=undefined){
					params['yonglimingcheng'] = '%' + this.searchForm.yonglimingcheng + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "ceshiyongli/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
