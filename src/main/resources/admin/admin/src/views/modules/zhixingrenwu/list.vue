<template>
	<div class="main-content" :style='{"padding":"20px 30px","margin":"0 0 0 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('zhixingrenwu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#FFFFFF","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zhixingrenwu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="ceshijieguo" label="测试结果">
						<template slot-scope="scope">
							<el-button v-if="scope.row.ceshijieguo" type="text" size="small" @click="download(scope.row.ceshijieguo)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' prop="baogaoquexian" label="报告缺陷">
						<template slot-scope="scope">
							<el-button v-if="scope.row.baogaoquexian" type="text" size="small" @click="download(scope.row.baogaoquexian)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zhixingrenwu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('zhixingrenwu','修复改进')" @click="xiufugaijinCrossAddOrUpdateHandler(scope.row,'cross','','','zhuangtai','已修复','已修复,待修复'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修复改进
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhixingrenwu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('zhixingrenwu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<xiufugaijin-cross-add-or-update v-if="xiufugaijinCrossAddOrUpdateFlag" :parent="this" ref="xiufugaijinCrossaddOrUpdate"></xiufugaijin-cross-add-or-update>
...
<script>
	...
	import xiufugaijinCrossAddOrUpdate from "../xiufugaijin/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				xiufugaijinCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			xiufugaijinCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.xiufugaijinCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zhixingrenwu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xiufugaijinCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xiufugaijinCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				if(this.searchForm.ceshiyongli!='' && this.searchForm.ceshiyongli!=undefined){
					params['ceshiyongli'] = this.searchForm.ceshiyongli
				}
